...
function it_exchange_print_paypal_pro_wizard_settings( $form ) {
    ...
    <?php } ?>
    <?php $IT_Exchange_PayPal_Pro_Add_On->get_paypal_pro_payment_form_table( $form, $form_values ); ?>
    </div>
    <?php
}
...
function it_exchange_save_paypal_pro_wizard_settings( $errors ) {
    ...
    $IT_Exchange_PayPal_Pro_Add_On = new IT_Exchange_PayPal_Pro_Add_On();
    return $IT_Exchange_PayPal_Pro_Add_On->paypal_pro_save_wizard_settings();
}
...
function it_exchange_paypal_pro_addon_default_settings( $values ) {
    $defaults = array(
        'paypal-pro-test-mode'             => false,
        'paypal-pro-sid'                   => '',
        'paypal-pro-secret'                => '',
        'paypal-pro-language'              => 'en',
        'paypal-pro-purchase-button-label' => __( 'Purchase', 'it-l10n-exchange-addon-paypal-pro' ),
    );
...
class IT_Exchange_PayPal_Pro_Add_On {
    ...
    */
    function __construct() {
        ...
        if ( ! empty( $_POST ) && $this->_is_admin && 'it-exchange-addons' == $this->_current_page && 'paypal-pro' == $this->_current_add_on ) {
    ...
    */
    function print_settings_page() {
        ...
        $form_options = array(
            'id'      => apply_filters( 'it_exchange_add_on_paypal_pro', 'it-exchange-add-on-paypal-pro-settings' ),
            'enctype' => apply_filters( 'it_exchange_add_on_paypal_pro_settings_form_enctype', false ),
            'action'  => 'admin.php?page=it-exchange-addons&add-on-settings=paypal-pro',
        );
        $form         = new ITForm( $form_values, array( 'prefix' => 'it-exchange-add-on-paypal-pro' ) );
        ...
        <div class="wrap">
            ...
            <?php $form->start_form( $form_options, 'it-exchange-paypal-pro-settings' ); ?>
                <?php do_action( 'it_exchange_paypal_pro_settings_form_top' ); ?>
                <?php $this->get_paypal_pro_payment_form_table( $form, $form_values ); ?>
     ...
     * @since 1.0.0
     */
    function get_paypal_pro_payment_form_table( $form, $settings = array() ) {
        ...
        <?php endif; ?>
        <div class="it-exchange-addon-settings it-exchange-paypal-pro-addon-settings">
            <p>
            ...
            <p>
                <label for="paypal-pro-api-username"><?php _e( 'API Username', 'it-l10n-exchange-addon-paypal-pro' ); ?> <span class="tip" title="<?php _e( 'Your PayPal Pro Account Number, or SID, is found in the top-right corner of your 2CO account dashboard.', 'it-l10n-exchange-addon-paypal-pro' ); ?>">i</span></label>
                <?php $form->add_text_box( 'paypal-pro-api-username' ); ?>
            </p>
            <p>
                <label for="paypal-pro-api-password"><?php _e( 'API Password', 'it-l10n-exchange-addon-paypal-pro' ); ?> <span class="tip" title="<?php _e( 'The PayPal Pro API Password is found in...', 'it-l10n-exchange-addon-paypal-pro' ); ?>">i</span></label>
                <?php $form->add_password( 'paypal-pro-api-password' ); ?>
            </p>
            <p>
                <label for="paypal-pro-api-signature"><?php _e( 'API Signature', 'it-l10n-exchange-addon-paypal-pro' ); ?> <span class="tip" title="<?php _e( 'The PayPal Pro API Password is found in...', 'it-l10n-exchange-addon-paypal-pro' ); ?>">i</span></label>
                <?php $form->add_password( 'paypal-pro-api-signature' ); ?>
            </p>
            ...
            <p class="hide-if-wizard">
                <?php $form->add_check_box( 'paypal-pro-sandbox-mode', array( 'class' => 'show-test-mode-options' ) ); ?>
                <label for="paypal-pro-sandbox-mode"><?php _e( 'Enable PayPal Pro Sandbox Mode?', 'it-l10n-exchange-addon-paypal-pro' ); ?> <span class="tip" title="<?php _e( 'Use this mode for testing your store. This mode will need to be disabled when the store is ready to process customer payments.', 'it-l10n-exchange-addon-paypal-pro' ); ?>">i</span></label>
            </p>

            <h4><?php _e( 'Optional: Edit Purchase Button Label', 'it-l10n-exchange-addon-paypal-pro' ); ?></h4>
            <p>
                <label for="paypal-pro-purchase-button-label"><?php _e( 'Purchase Button Label', 'it-l10n-exchange-addon-paypal-pro' ); ?> <span class="tip" title="<?php _e( 'This is the text inside the button your customers will press to purchase with PayPal Pro', 'it-l10n-exchange-addon-paypal-pro' ); ?>">i</span></label>
                <?php $form->add_text_box( 'paypal-pro-purchase-button-label' ); ?>
            </p>
        </div>
        <?php
    }
    ...
    */
    function save_settings() {
        ...
        // Check nonce
        if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'it-exchange-paypal-pro-settings' ) ) {
    ...
    */
    function paypal_pro_save_wizard_settings() {
        ...
        $fields = array(
            ...
            'paypal-pro-sandbox-mode',
            'paypal-pro-purchase-button-label'
        );
