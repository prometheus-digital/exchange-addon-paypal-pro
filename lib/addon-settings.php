    ...
    function get_form_table( $form, $settings = array() ) {
        ...
        <div class="it-exchange-addon-settings it-exchange-paypal_pro-addon-settings">
            ...
            <?php
                ...
                $license = $exchangewp_paypal_pro_options['paypal_pro_license'];
                // var_dump($license);
                $exstatus = trim( get_option( 'exchange_paypal_pro_license_status' ) );
                // var_dump($exstatus);
             ?>
            <p>
              <label class="description" for="exchange_paypal_pro_license_key"><?php _e('Enter your license key'); ?></label>
              <!-- <input id="paypal_pro_license" name="it-exchange-add-on-paypal_pro-paypal_pro_license" type="text" value="<?php #esc_attr_e( $license ); ?>" /> -->
              <?php $form->add_text_box( 'paypal_pro_license' ); ?>
              <span>
                <?php if( $exstatus !== false && $exstatus == 'valid' ) { ?>
    							<span style="color:green;"><?php _e('active'); ?></span>
    							<?php wp_nonce_field( 'exchange_paypal_pro_nonce', 'exchange_paypal_pro_nonce' ); ?>
    							<input type="submit" class="button-secondary" name="exchange_paypal_pro_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
    						<?php } else {
    							wp_nonce_field( 'exchange_paypal_pro_nonce', 'exchange_paypal_pro_nonce' ); ?>
    							<input type="submit" class="button-secondary" name="exchange_paypal_pro_license_activate" value="<?php _e('Activate License'); ?>"/>
    						<?php } ?>
              </span>
            </p>
    ...
    */
    function save_settings() {
        ...
        // listen for our activate button to be clicked
      	if( isset( $_POST['exchange_paypal_pro_license_activate'] ) ) {

      		// run a quick security check
      	 	if( ! check_admin_referer( 'exchange_paypal_pro_nonce', 'exchange_paypal_pro_nonce' ) )
      			return; // get out if we didn't click the Activate button

      		// retrieve the license from the database
      		// $license = trim( get_option( 'exchange_paypal_pro_license_key' ) );
          $exchangewp_paypal_pro_options = get_option( 'it-storage-exchange_addon_paypal_pro' );
          $license = trim( $exchangewp_paypal_pro_options['paypal_pro_license'] );

      		// data to send in our API request
      		$api_params = array(
      			'edd_action' => 'activate_license',
      			'license'    => $license,
      			'item_name'  => urlencode( 'paypal-pro' ), // the name of our product in EDD
      			'url'        => home_url()
      		);
      			...
      			if ( false === $license_data->success ) {
      				...
      				switch( $license_data->error ) {
      					...
      					case 'item_name_mismatch' :
      						...
      						$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), 'paypal_pro' );
      						break;
        ...
        // deactivate here
        // listen for our activate button to be clicked
      	if( isset( $_POST['exchange_paypal_pro_license_deactivate'] ) ) {
