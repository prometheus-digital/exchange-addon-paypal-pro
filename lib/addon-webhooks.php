...
add_filter( 'init', 'it_exchange_paypal_pro_addon_register_webhook' );
 ...
 * @since 0.4.0
 *
 * @param integer $paypal_pro_id id of paypal transaction
 ...
 * @return bool
*/
function it_exchange_paypal_pro_addon_add_child_transaction( $paypal_pro_id, $payment_status, $subscriber_id=false, $amount ) {
	...
	if ( !empty( $transactions ) ) {
		...
		it_exchange_paypal_pro_addon_update_transaction_status( $paypal_pro_id, $payment_status );
	} else {
		...
		if ( $parent_tx_id && $customer_id ) {
			...
			it_exchange_add_child_transaction( 'paypal_pro', $paypal_pro_id, $payment_status, $customer_id, $parent_tx_id, $transaction_object );
			return true;
		}
	}
	return false;
 ...
 *
 * @since 1.3.0
 * @param string $paypal_pro_id PayPal Transaction ID
